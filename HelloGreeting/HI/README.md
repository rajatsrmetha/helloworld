...
## Building and Running the HI Application

In order to build the HI application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the HI application's code.
```
fprime-util build
```
...
To run the ground system without starting the HI app:
```
...
```
cd HI/build-artifacts/<platform>/bin/
./HI -a 127.0.0.1 -p 50000
```
