...
In order to build the HELLO application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the HELLO application's code.
```
fprime-util build
```
...
To run the ground system without starting the HELLO app:
```
...
```
cd HELLO/build-artifacts/<platform>/bin/
./HELLO -a 127.0.0.1 -p 50000
```
